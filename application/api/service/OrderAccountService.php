...
use app\api\model\OrderAccountModel;
class OrderAccountService {
    ...
    public function add($total_fee = 0, $pay_type = 1, $type=1, $uid =[]){
        ...
        try{
            ...
            // // 添加充值流水
            // $type = config('running_type.recharge');
            // $options['uid'] = $uid;
            // $options['title'] = ($type==1) ? '账户充值' : '账户提现';
            // // 支付方式
            // $options['pay_intro'] = $pay_info[$pay_type];
            // $res = RunningModel::self()->add($pay_type, $type, $total_fee, $uid, $options);
            // if(!$res){
            //     Db::rollback();    
            //     return false;
            // }
