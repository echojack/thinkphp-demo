...
use app\api\model\CallbackModel;
...
class PayService {
     ...
     */
    public function call($pay_type = 1,$order = []){
        switch ($pay_type) {
            case 1:
                $notify_url = url('Notify/wx_recharge_callback', '', false, true);
                // $detail = $this->_wx_order($order, $notify_url);
                break;
            case 2:
    ...
    private function _alipay_order($param = [], $notify_url = ''){
        ...
        \think\Log::write($response,'alipay_sign');
	return $response;
     ...
     * 支付宝同步数据检测
     */
    public function recharge_search($resultStatus = '', $param = []){
        $response = $param['alipay_trade_app_pay_response'];
        switch ($resultStatus) {
            case '9000'://订单支付成功
                $res = OrderService::self()->alipay_recharge_success($response);
                break;
            case '5000'://重复请求
    ...
    public function alipay_order_search( $resultStatus = '',$param = ''){
        ...
        switch ($resultStatus) {
            case '9000'://订单支付成功
                $res = OrderService::self()->alipay_order_success($response);
                break;
            case '5000'://重复请求
            case '8000'://正在处理中，支付结果未知（有可能已经支付成功），请查询商户订单列表中订单的支付状态
            case '6004'://支付结果未知（有可能已经支付成功），请查询商户订单列表中订单的支付状态
