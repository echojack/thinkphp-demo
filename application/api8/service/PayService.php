...
use app\api8\model\CallbackModel;
...
class PayService {
    ...
    public function alipay_order_search( $resultStatus = '',$param = ''){
        ...
        // 支付失败，修改订单状态为失败
        if($res !== true){
            if(isset($param['alipay_trade_app_pay_response'])){
                $result = $param['alipay_trade_app_pay_response'];
                if(isset($result['out_trade_no'])){
                    $out_trade_no = $result['out_trade_no'];
                    $this->order_pay_error($out_trade_no);
                }
            }
        }
        return $res;
    ...
    public function order_pay($order = [], $pay_type = 1){
        ...
        $param['subject'] = $pay_type == 1 ? '微信订单支付' : '支付宝订单支付';
    ...
    public function wx_recharge_callback($data){
        ...
        $stringSignTemp = $buff . 'key=50c6077aa65f11bd17b64cfd5707a42d';//key为证书密钥
