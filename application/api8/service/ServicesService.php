...
class ServicesService {
     ...
     * 服务列表
     */
    public function service_lists($param = [], $page = 0, $limit = 10){
        // 排序处理
        $param['type'] = 1;
        $param['status'] = 1;
        // 黑名单过滤
    ...
    private function _common_data($id = '', $uid = ''){
        ...
        if($created_user){
            ...
            $tmp_detail['nick_name'] = isset($created_user['nick_name']) ? $created_user['nick_name'] : '';
            $tmp_detail['sex'] = isset($created_user['sex']) ? $created_user['sex'] : '';
            $tmp_detail['birthday'] = isset($created_user['birthday']) ? $created_user['birthday'] : '';
            $tmp_detail['avatar'] = isset($created_user['avatar']) ? $created_user['avatar'] : '';
            $tmp_detail['age'] = isset($created_user['age']) ? $created_user['age'] : '';
        }
        return $tmp_detail;
    }
    ...
    /**
     * NEW 首页调用 多个services   id来回调用
     */
    public function _common_data_index($id = '',$uid = '')
    {
        $where['id'] = $id;
        $detail = ServiceModel::self()->detail($where);
        $tmp_detail = [];
        ...
        //用户信息
        if ($tmp_detail) {
            $created_user = UserService::self()->detail($tmp_detail['created_uid'], '');
            if ($created_user) {
                $tmp_detail['uid'] = $tmp_detail['created_uid'];
                $tmp_detail['nick_name'] = isset($created_user['nick_name']) ? $created_user['nick_name'] : '';
                $tmp_detail['sex'] = isset($created_user['sex']) ? $created_user['sex'] : '';
            ...
            //服务类型
            $services_type_data = ServiceModel::self()->services_type_data($tmp_detail['category_id']);
            $tmp_detail['value'] = $services_type_data['value'];
        }
        return $tmp_detail;
    }

    /**
     * NEW 首页列表 多个services.id
     */
    public function index_list($page = 1, $limit = 10)
    {
        $list = ServiceModel::self()->index_list($page,$limit);
        $tmp_list = [];
        if ($list)
        {
            foreach ($list as $id)
            {
                $detail = $this->service_detail_index($id,'1',false);
                if ($detail)
                {
                    $tmp_list[] = $detail;
                }
    ...
    /**
     * NEW 首页调用 多个services.id
     */
    public function service_detail_index($id = '', $uid = '', $flag = true)
    {
        if(!$id){
            return [];
        }
        $tmp_detail = $this->_common_data_index($id, $uid);
        return $tmp_detail;
    }

    //分类调用
    public function index_type_list_details($id = '')
    {
        $where['id'] = $id;
        $detail = ServiceModel::self()->detail($where);
        $tmp_detail = [];
...
//        //分类
//        $category_list = ConfigModel::self()->get_parents_id($tmp_detail['category_id']);
//        $id_list = Db::table('configs')->field('configs_id,value')->where('parent','=',$category_list)->select();
//        if($id_list){
//            foreach ($id_list as $k => $val) {
//                $id_list[$k];
//            }
//        }
//        $tmp_detail['value'] = $id_list;
        return $tmp_detail;
    }
    ...
    {
        $list = ServiceModel::self()->index_type($type,$category_id,$page,$limit,$sex,$time,$order);
        $tmp_list = [];
        if ($list)
        {
            foreach ($list as $id)
            {
                $detail = $this->index_type_details($id);
                if ($detail)
                {
                    $tmp_list[] = $detail;
                }
     ...
     * @return array
     */
    public function index_type_details($id = '')
    {
        if(!$id){
            return [];
        }
        $tmp_detail = $this->index_type_list_details($id);
        return $tmp_detail;
    }
    /*
     * 分类
    ...
    public function Type_list($type = '')
    {
        ...
        $list = ConfigModel::self()->Type_list($type);
        return $list;
    }
