...
class OrderModel extends Model{
    ...
    public function del_order($order_id = '', $user = []){
        ...
        $order = $this->where($o_where)->field('created_uid')->find();
        if(!$order){
            return false;
        }
        if($order['created_uid'] == $user['uid']){
            $save['is_del'] = 1;
        }else{
            $save['status'] = 6;
        }
