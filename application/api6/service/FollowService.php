...
use app\api6\service\PushService;
use app\api6\model\FollowModel;
class FollowService {
    ...
    /**
     * 关注用户处理
     * @param follow_id 多个用户使用‘,’连接
     */
    public function dofollows($follow_ids = '', $follower_id = ''){
        $follow_ids = str_replace('，', ',', $follow_ids);
        $tmp = explode(',', $follow_ids);
        foreach ($tmp as $follow_id) {
            $this->dofollow($follow_id, $follower_id);
        }
        return true;
    }
     /**
