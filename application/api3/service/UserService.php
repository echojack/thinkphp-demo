...
use app\api3\model\UserAccountModel;
...
use app\api3\service\OrderAccountService;
...
class UserService {
    ...
    public function get_user_base_info($uid = '', $current_uid = ''){
        ...
        // 计算用户距离
        if($return && $current_uid){
            if($uid == $current_uid){
                $return['distance'] = 0;
            }else{
                $user = Db::name('users_ext')->where(['uid'=>$current_uid])->find();
                $return['distance'] = getDistance($user['latitude'], $user['longitude'], $return['latitude'], $return['longitude']);    
            }
        }
        return $return;
    }
