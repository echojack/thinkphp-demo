...
use think\Config;
use think\Cache;
use app\api3\model\LogsModel;
use app\api3\model\OrderModel;
use app\api3\model\ServicesModel;
use app\api3\service\UserService;
use app\api3\service\PayService;
use app\api3\model\RunningModel;
...
class OrderService {
    ...
    public function post_service($param = [], $user = []){
        ...
        $service = ServicesModel::self()->where($where)->find();
        if(!$service){
    ...
    public function post_demand($param = [], $user = []){
        $demand = ServicesModel::self()->demand_detail($param['id']);
        if(!$demand){
    ...
    public function detail($order_id = ''){
        ...
        if(!$order){
            ...
            if($order['source_type'] == 1){
                ...
                $order['time'] = demandTime($order['source_id']);    
            }
        ...
        $order['copy'] = $this->copy_detail($copy_id);
        return $order;
    }
    /**
     * 服务详情展示
     */
    public function copy_detail($copy_id = ''){
        if(!$copy_id){
            return [];
        }
