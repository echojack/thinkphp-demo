...
use app\api4\service\PayService;
...
class Pay extends ApiLogin {
    ...
    public function withdraw(){
        ...
        if($pay !== true){
            $remarks = ['param'=>$param, 'intro'=>'提现失败'];
            LogsService::self()->add($this->user['uid'], $this->user['nick_name'], ADD_ACT, 0, OTHER, SUC_ACT, $remarks);
            $this->wrong(0, $pay);
        }
        $remarks = ['param'=>$param, 'intro'=>'提现成功'];
        LogsService::self()->add($this->user['uid'], $this->user['nick_name'], ADD_ACT, 0, OTHER, SUC_ACT, $remarks);
    ...
    public function alipay_order(){
        ...
        // 创建支付订单
        $pay = PayService::self()->order_pay($order_id, 2);
        if($pay === false){
    ...
    public function wx_order(){
        ...
        // 创建支付订单
        $pay = PayService::self()->order_pay($order_id, 1);
        if($pay === false){
            $this->wrong(0, '支付失败，请刷新再试');
        }
        $return['pay'] = $pay;
