...
class UploadModel extends Model{
     ...
     * 多张图片上传
     */
    public function arr_upload($files = [], $upload_path = '', $flag = 0){
        $tmp_attach = [];
     ...
     * 生成图片缩略图
     */
    public function make_thum_images($info_image = [], $width = 300, $height = 300){
        $img_width = $info_image['width'];
        $img_path = \think\Config::get('upload_path').$info_image['path'];
        // 缩略图命名
        $tmp = explode('/', $info_image['path']);
        $tmp[count($tmp) - 1] = 'thum_'.$tmp[count($tmp) - 1];
        $thum_name = \think\Config::get('upload_path').implode('/', $tmp);

        // 缩略图url 
        $thumb_url = \think\Config::get('img_url').$info_image['path'];
        if(is_file($thum_name)){
            return $thumb_url;
        }else{
            $image    = Image::open($img_path);
            if($img_width > $width){
                $result = $image->thumb($width, $height, 1)->save($thum_name);
                if($result !== false){
                    $thumb_url = \think\Config::get('img_url').implode('/', $tmp);
                }
            }
        }
        return $thumb_url;
